...
def integrate_one(x0, T_min, T_max, dt, events=None, rtol=1e-9, theta_s_0=0):
...
def shooting_residual(delta_v):
    ...
    trial_solution = solve_ivp(
        Integrator.BCR4BP_ODE,
        [0, transfer_duration],
        x0_trial,
        args=(mu, mu_s, r_s, omega_s, omega_s * departure_time / TU),
        method="DOP853",
        rtol=1e-10,
        atol=1e-12,
...
def homotopy_ode(t, x, mu, strength):
    ...
    # Gradually add the Moon and Sun gravity
    full = Integrator.BCR4BP_ODE(t, x, mu, mu_s, r_s, omega_s, omega_s * departure_time / TU)
    dxdt[3:] += strength * (full[3:] - dxdt[3:])
...
def process_trajectory(current_row):
    ...
    if PRINT or PLOT:
        ...
        transfer_result = integrate_one(x_departure_normalized, 0, transfer_duration, dt_transfer, events=None, theta_s_0=omega_s * departure_time / TU)
    ...
    transfer_result = integrate_one(x_departure_normalized_refined, 0, transfer_duration, dt_transfer, events=None, rtol=1e-10, theta_s_0=omega_s * departure_time / TU)
...
# Input
DATA_FILE = "/home/lucacecca/Astrodynamics/BCR4BP/Generation/Escape_initial_conditions_CR3BP_solar.txt"
...
if SAVE:
    ...
    if output_rows:
        ...
        np.savetxt(
            "/home/lucacecca/Astrodynamics/BCR4BP//Escape_transfers_database_solar.txt",
            data,
            header=(
                "t_departure_s "
